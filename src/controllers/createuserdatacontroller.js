...
const CreateUserDataController = catchAsync(
    async (req, res) => {
        ...
        for (let user of jsonUsers) {
            ...
            const queriedUser = await User.findOne({email: userEmail});
            if (!queriedUser) {
        ...
        // maillist = JSON.stringify(maillist, null, 2);
        // fs.writeFileSync('newuseremails.json', maillist);
        // res.download('newuseremails.json');
